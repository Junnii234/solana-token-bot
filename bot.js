...
// UPDATED: Added Satoshi's funder signature (6a7S2)
...
bot.onText(/\/test (.+)/, async (msg, match) => {
    ...
    bot.sendMessage(msg.chat.id, `🔍 Testing ${testMint.substring(0,8)}...`);
    ...
    } catch (e) { bot.sendMessage(msg.chat.id, "❌ Error fetching dev wallet."); }
});
...
async function performForensic(mint, devWallet, isManual = false) {
    try {
        ...
        // Match logic from bot (5).js
        const isCEX = CEX_LIST.some(sig => funder.startsWith(sig) || logs.includes(sig.toLowerCase()));

        if (isCEX) {
            bot.sendMessage(TELEGRAM_CHAT_ID, `✅ *TEST PASSED*\nMint: \`${mint}\`\nFunding: *CEX Verified*`, {parse_mode: 'Markdown'});
        } else if (isManual) {
            bot.sendMessage(TELEGRAM_CHAT_ID, `❌ *TEST FAILED*\nFunder: \`${funder.substring(0,8)}...\` (Not in CEX list)`);
        }
    } catch (e) { }
}
