...
// ================= CORE FORENSIC ENGINE (V21-FIXED) =================

async function performFullForensic(mint) {
    try {
        const now = Math.floor(Date.now() / 1000);

        // 1. Get Pair Age & Signature (Oldest First) - INCREASED LIMIT
        const pairSigs = await axiosWithRetry({ 
            jsonrpc: "2.0", 
            id: 1, 
            method: "getSignaturesForAddress", 
            params: [mint, { limit: 10, oldestFirst: true }] // Increased from 1 to 10
        });
        
        // FILTER: Only signatures with valid blockTime
        const validPairSigs = pairSigs.data.result.filter(tx => tx.blockTime !== null && tx.blockTime !== undefined);
        const firstPairTx = validPairSigs[0];
        
        if (!firstPairTx) {
            return { error: "Mint not found on-chain or no valid blockTime" };
        }
        ...
        if (!creator) {
            return { error: "Could not extract creator from transaction" };
        }
        ...
        // 4. Dev Age Logic - FIXED WITH FILTERING
        const devNewSigs = await axiosWithRetry({ 
            jsonrpc: "2.0", 
            id: 1, 
            method: "getSignaturesForAddress", 
            params: [creator, { limit: 10 }] // Increased from 1 to 10
        });
        ...
        const devOldSigs = await axiosWithRetry({ 
            ...
            id: 1, 
            method: "getSignaturesForAddress", 
            params: [creator, { limit: 10, oldestFirst: true }] // Increased from 1 to 10
        });
        
        // FILTER: Only valid blockTimes
        const validDevNewSigs = devNewSigs.data.result.filter(tx => tx.blockTime !== null && tx.blockTime !== undefined);
        const validDevOldSigs = devOldSigs.data.result.filter(tx => tx.blockTime !== null && tx.blockTime !== undefined);
        
        if (validDevNewSigs.length === 0 || validDevOldSigs.length === 0) {
            return { error: "Could not retrieve valid dev transaction history" };
        }
        
        const devNewestTime = validDevNewSigs[0].blockTime;
        const devOldestTime = validDevOldSigs[0].blockTime;
        ...
        const firstDevTx = await axiosWithRetry({ 
            ...
            id: 1, 
            method: "getTransaction", 
            params: [validDevOldSigs[0].signature, { maxSupportedTransactionVersion: 0, encoding: "jsonParsed" }] 
        });
...
bot.onText(/\/test (.+)/, async (msg, match) => {
    ...
    let report = `📊 **FORENSIC REPORT (V21-FIXED)**\n\n`;
...
function startMigrationListener() {
    ...
    ws.on("open", () => {
        console.log("🛡️ V21-FIXED Radar Online");
        ...
        if (e.txType === 'raydium_migration') {
            ...
            if (!res.error && res.pass) {
                bot.sendMessage(TELEGRAM_CHAT_ID, 
                    ...
                    `**Auth:** ✅ Clean\n` +
                    `**Reason:** ${res.isCEX ? "CEX Funded" : parseFloat(res.devFirstActionAge) >= 90 ? "Established Dev" : "Active Dev"}\n\n` +
...
console.log("BOT RUNNING - V21-FIXED - BLOCKTIME FILTERING ENABLED");
